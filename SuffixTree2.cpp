...
/*
   for each suffix:
	if not found in list: make new node(suffix, nullptr) and append
	else if matching prefix:
		extract prefix = matching prefix;
		oldNode(prefix);
		oldNode->child add(remSuffixList)
		oldNode ->child add(substr)
